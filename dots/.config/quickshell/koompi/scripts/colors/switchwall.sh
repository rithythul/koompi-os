...
pre_process() {
    ...
    if [[ "$mode_flag" == "dark" ]]; then
        ...
        gsettings set org.gnome.desktop.interface icon-theme 'breeze-plus'
    fi
...
set_wallpaper_path() {
    local path="$1"
    if [ -f "$SHELL_CONFIG_FILE" ]; then
        if jq --arg path "$path" '.background.wallpaperPath = $path' "$SHELL_CONFIG_FILE" > "$SHELL_CONFIG_FILE.tmp" && jq -e 'type == "object"' "$SHELL_CONFIG_FILE.tmp" >/dev/null; then
...
set_thumbnail_path() {
    local path="$1"
    if [ -f "$SHELL_CONFIG_FILE" ]; then
        if jq --arg path "$path" '.background.thumbnailPath = $path' "$SHELL_CONFIG_FILE" > "$SHELL_CONFIG_FILE.tmp" && jq -e 'type == "object"' "$SHELL_CONFIG_FILE.tmp" >/dev/null; then
            mv "$SHELL_CONFIG_FILE.tmp" "$SHELL_CONFIG_FILE"
        else
            rm -f "$SHELL_CONFIG_FILE.tmp"
        fi
...
main() {
    ...
    set_accent_color() {
        local color="$1"
        if jq --arg color "$color" '.appearance.palette.accentColor = $color' "$SHELL_CONFIG_FILE" > "$SHELL_CONFIG_FILE.tmp" && jq -e 'type == "object"' "$SHELL_CONFIG_FILE.tmp" >/dev/null; then
            mv "$SHELL_CONFIG_FILE.tmp" "$SHELL_CONFIG_FILE"
        else
            rm -f "$SHELL_CONFIG_FILE.tmp"
        fi
